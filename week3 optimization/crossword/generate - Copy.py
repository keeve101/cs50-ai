...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        """
        for v1, words in self.domains.items():
            incons_words = []
            for word in words:
                if len(word) != v1.length:
                    incons_words.append(word)
            for word in incons_words:
                self.domains[v1].remove(word)
        ...
        while arcs:
            ...
            if self.revise(v1, v2):
                if not self.domains[v1]:
                    return False
                neighbours = self.crossword.neighbors(v1)
                for v3 in neighbours:
                    arcs.append((v3, v1))
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.domains.keys():
            if var not in assignment:
                domain_no = len(self.domains[var])
                if lowest > domain_no:
                    lowest = domain_no
                    chosen = var
                elif lowest == domain_no:
                    if chosen:
                        if len(self.crossword.neighbors(chosen)) < len(self.crossword.neighbors(var)):
                            chosen = var
